...
fn player_collision(
    ...
    mut player_query: Query<
        ...
        With<PlayerEntity>,
    >,
    wall_query: Query<(Entity, &RectangleHitboxCmp, &Handle<ColorMaterial>), With<WallEntity>>,
) {
    if let Ok((mut player_mov, player_hitbox, mut player_tf)) = player_query.single_mut() {
        ...
        for (wall_entity, wall_hitbox, wall_color) in wall_query.iter() {
            ...
            if collided {
                ...
                if contact_normals != contact_normals_new {
                    let resolving_side = u32_to_rect_collision_side(contact_normals ^ contact_normals_new);
                    let resolve = rect_collision_side_to_vec2(resolving_side)
                    * player_mov.speed
                    * (1_f32 - contact_time.unwrap())
                    * TIME_STEP;
                    dbg!((delta, resolve, resolving_side));
                    delta += resolve;
                } 
                contact_normals = contact_normals_new;
                
                // dbg!(player_mov.position);
                // dbg!(contact_time.unwrap());
            } else {
...
fn player_update_hitbox(
    mut player_query: Query<(&mut MovementCmp, &mut RectangleHitboxCmp), With<PlayerEntity>>,
) {
    if let Ok((mut player_mov, mut player_hitbox)) = player_query.single_mut() {
        player_hitbox
            .rect
